...
-y | --starGTF # The format of this argument is "GTF /path/to/my/file.gtf" IMPORTANT: the letters GTF must be included before the filepath. The .gtf file used should be the genome annotation .gtf file that corresponds to the STAR index genome. This is not a file generated by STAR but just a GTF format file downloaded from the same place you downloaded your raw genome. IMPORTANT: Use either a .gtf index using -y or a .saf index using -s. Do not use both.

-s | --geneAnnotationSAF # The format of this argument is "SAF /path/to/my/file.gtf" IMPORTANT: the letters SAF must be included before the filepath. The .saf file must corresponds to the genome used to construct your STAR index. SAF format annotation files are preferred for nuclei sequencing when the user wants to assign both intronic and exonic reads to genes. Using a GTF format file will only facilitate assignment of exonic mapping reads and may result in a low percentage of reads assigned to genes. Prebuild indexes are provide for Human (GRCh38) and Mouse (GRCm38) from Ensembl.org. Provide the full filepath to the appropriate .saf index. 
IMPORTANT: Use either a .gtf index using -y or a .saf index using -s. Do not use both.
...
`bash splitseqdemultiplex.sh -n 4 -e 1 -m 10 -1 Round1_barcodes_new5.txt -2 Round2_barcodes_new4.txt -3 Round3_barcodes_new4.txt -f SRR6750041_1_smalltest.fastq -r SRR6750041_2_smalltest.fastq -o results -t 8000 -g 100000 -c true -a star -x ~/my/path/to/starIndexDirectory/GRCm38/ -s SAF ~/my/path/to/geneAnnotation/GRCm38_genes.saf`
