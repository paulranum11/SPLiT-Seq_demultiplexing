...
-y | --starGTF # provide the path to the genome annotation .gtf file that corresponds to the STAR index genome. This is not a file generated by STAR but just a GTF format file downloaded from the same place you downloaded your raw genome.

-s | --geneAnnotationSAF # provide the path to the genome annotation .saf file that corresponds to the genome used to construct your STAR index. SAF format annotation files are preferred for nuclei sequencing when the user wants to assign both intronic and exonic reads to genes. Using a GTF format file will only facilitate assignment of exonic mapping reads and may result in a low percentage of reads assigned to genes. Prebuild indexes are provide for Human (GRCh38) and Mouse (GRCm38) from Ensembl.org. Provide the full filepath to the appropriate .saf index. 
