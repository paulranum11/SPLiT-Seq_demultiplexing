...
then
    ...
    # Create a "heavy mode" that can help overcome the limitations of bash pipes for very high numbers of identified cells
    if [ $HEAVY = true ]
    then
        # This is an attempt bin the execution of parallel calls to Extract_BC_UMI.py
        find . -iname "*.fastq" > full_fastq_list.txt
        fliename="full_fastq_list.txt"
        counter=0    
        while read line; do
            #echo "$line"
            if [ "$counter" -lt 100 ] 
            then
                basename "$line" >> intermediate_fastq_list.txt
                counter=$[counter + 1]
            else
                basename "$line" >> intermediate_fastq_list.txt
                parallel python ../Extract_BC_UMI.py -R {} -F {}-MATEPAIR ::: $(cat intermediate_fastq_list.txt)
                counter=0
                rm intermediate_fastq_list.txt
            fi
        done < full_fastq_list.txt
    ...
    if [ $HEAVY = true ]
    then 
        find . -iname "*-A*_1.fastq" | parallel rm {}
        find . -iname "*-C*_1.fastq" | parallel rm {}    
        find . -iname "*-T*_1.fastq" | parallel rm {}
        find . -iname "*-G*_1.fastq" | parallel rm {}

        find . -iname "*-A*.fastq" | parallel rm {}
        find . -iname "*-C*.fastq" | parallel rm {}
        find . -iname "*-T*.fastq" | parallel rm {}
        find . -iname "*-G*.fastq" | parallel rm {}
    
        find . -iname "*-A*.fastq-MATEPAIR" | parallel rm {}
        find . -iname "*-C*.fastq-MATEPAIR" | parallel rm {}
        find . -iname "*-T*.fastq-MATEPAIR" | parallel rm {}
        find . -iname "*-G*.fastq-MATEPAIR" | parallel rm {}
    else
        ls | grep '\.fastq*' | parallel rm {}
    fi

    rm intermediate_fastq_list.txt
    rm full_fastq_list.txt
